...
import OnlineCoursesAdd from "./OnlineCoursesAdd";
import { Download, PlusCircle } from "phosphor-react";
import OnlineCoursesUpdate from "./OnlineCoursesUpdate";
...
function OnlineCourses() {
  ...
  const fetchCourses = () => {
    ...
    axios.get(`${BASE_URL}/onlinecourses.php`)
      .then(response => {
        ...
        localStorage.setItem("onlinecourses",numberOfLines);
      })
      .catch(error => {
  ...
  return (
    <>
      ...
      {showAdd && <OnlineCoursesAdd handleCancel={openAddCourseCancel} />}

      {showUpdate && <OnlineCoursesUpdate editId={onlineCoursesEditId} handleCancel={openUpdateCourseCancel} />}
    </>
  );
}
